...
class BaseLerobotDataset(torch.utils.data.Dataset):
    ...
    def _record_bad_video_sample(self, sample_idx: int, err: Exception, attempt: int) -> None:
        err_str = str(err)
        if "Failed to decode video frames" not in err_str:
            return

        log_path = os.environ.get("FASTWAM_BAD_VIDEO_LOG")
        if log_path is None:
            work_dir = misc.get_work_dir()
            log_path = str(Path(work_dir) / "bad_video_samples.jsonl")
        ...
        try:
            ...
            logger.warning("Failed to write bad video sample log to %s: %s", log_path, write_err)
    ...
    def __getitem__(self, idx):
        ...
        while attempt < MAX_GETITEM_ATTEMPT:
            try:
                ...
                attempt += 1
                last_exception = err
                self._record_bad_video_sample(sample_idx, err, attempt)
                logger.warning(
