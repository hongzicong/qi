...
namespace cutlass_int8_sm8x_t64x256 {
    ...
    cudaStream_t stream)
{
    ...
    if (st != cutlass::Status::kSuccess) {
        std::fprintf(stderr,
                     "[qi_cutlass_int8_sm8x_t64x256] can_implement failed: M=%d N=%d K=%d code=%d\n",
    ...
    if (st != cutlass::Status::kSuccess) {
        std::fprintf(stderr,
                     "[qi_cutlass_int8_sm8x_t64x256] init failed: M=%d N=%d K=%d code=%d\n",
                     M, N, K, static_cast<int>(st));
    ...
    cudaStream_t stream)
{
    ...
    if (st != cutlass::Status::kSuccess) {
        std::fprintf(stderr,
                     "[qi_cutlass_int8_sm8x_t64x256_bias] can_implement failed: M=%d N=%d K=%d code=%d\n",
                     M, N, K, static_cast<int>(st));
        return static_cast<int>(st) | 0x10000;
    }
    ...
    if (st != cutlass::Status::kSuccess) {
        std::fprintf(stderr,
                     "[qi_cutlass_int8_sm8x_t64x256_bias] init failed: M=%d N=%d K=%d code=%d\n",
                     M, N, K, static_cast<int>(st));
        return static_cast<int>(st) | 0x20000;
    }
...
extern "C" int qi_cutlass_int8_rowwise_bf16out_t64x256(
    ...
    cudaStream_t stream)
{
    return qi::quantization::cutlass_int8_sm8x_t64x256::run(
    ...
    cudaStream_t stream)
{
    ...
    return qi::quantization::cutlass_int8_sm8x_t64x256::run_bias(
