...
from qi.datasets.lerobot.processors.fastwam_processor import FastWAMProcessor
...
class WorldActionRobotWinPolicy:
    def __init__(
        ...
        self.expert_cache = bool(expert_cache)
            ...
            infer_kwargs.update(
                {
                    ...
                    "expert_cache_reuse_steps": self.expert_cache_reuse_steps,
                    "expert_cache_warmup_steps": self.expert_cache_warmup_steps,
        ...
        if not self.pending_actions:
            if observation is None:
                raise ValueError(
                    ...
                    "(replan step for qi RobotWin policy)."
                )
...
def get_model(usr_args: Dict[str, Any]):
    ...
    expert_cache = _parse_bool(
        usr_args.get("expert_cache", cfg.EVALUATION.get("expert_cache", False))
    )
    expert_cache_reuse_steps = int(
        usr_args.get("expert_cache_reuse_steps", cfg.EVALUATION.get("expert_cache_reuse_steps", 1))
    )
    expert_cache_warmup_steps = int(
        usr_args.get("expert_cache_warmup_steps", cfg.EVALUATION.get("expert_cache_warmup_steps", 1))
    )
    expert_cache_cooldown_steps = int(
